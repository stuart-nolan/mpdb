...
Ref:
    #vapor viscosity algorithm validation
    moleFrac = [.697, 0.303]    
    fW = [16.043,58.123]
    pCVL = [109.4, 72.74]
    pCTCL = [0.0166,0.0214]
    ...
    for idx in range(nMF): #row
        ...
        for jdx in range(nMF): #col
            fWR=fW[idx]/fW[jdx]
            vR = pCVL[idx]/pCVL[jdx]
            if idx == jdx:
                sumyjAij += moleFrac[jdx]
            elif idx < jdx:
                Aij[idx].append((1+vR**(1./2)*(1/fWR)**(1./4))**2/(8*(1+fWR))**(1./2))
                sumyjAij += moleFrac[jdx]*Aij[idx][jdx-(1+idx)]
            else:
                Aij[idx].append(Aij[jdx][idx-(1+jdx)]*vR/fWR)
                sumyjAij += moleFrac[jdx]*Aij[jdx][idx-(1+jdx)]*vR/fWR
        tCm += moleFrac[idx]*pCVL[idx]/sumyjAij
"""
...
cpdef shomate_enthalpy_bounded(float T, list TBounds, list sPs):
    """
    shomate_enthalpy_bounded(float T, list Tbounds, list sPs)
   ...
   Returns
      shomate_enthalpy(T, sP[index]) if T is within TBounds otherwise
      None    
    """
    for idx, TB in enumerate(TBounds):
        if T >= TB[0] and T <= TB[1]:
            return shomate_enthalpy(T, sPs[idx])

    return None

cpdef float shomate_enthalpy(float T, list sP):
    """
    shomate_enthalpy(float T, list sP)
    
    Enthalpy at T relative to enthalpy at 298.15 from Nist Shomate equation
      t=T/1000
      tref=TRef/1000
      shomate_enthalpy = 1000*( A*t + B/2*t^2 + C/3*t^3 + D/4*T^4 - E*1/t +
                                F - H )
    Parameters
      T, temperature in Kelvin
    ...
    Returns
      enthalpy change in J/mol between T and TRef = 298.15 K
    """
    t=T/1000
    return 1000*(sP[0]*t + sP[1]/2.0*t**2 + sP[2]/3.0*t**3 + sP[3]/4.0*t**4 - sP[4]/t + sP[5] - sP[7])
